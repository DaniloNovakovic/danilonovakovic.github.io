...
Runtime layering follows a micro-kernel + bridge pattern. Feature runtime lookup lives in [`src/game/runtime/miniGameRegistry.ts`](src/game/runtime/miniGameRegistry.ts), with source facts owned by [`src/features`](src/features) and composed through the feature catalog and registry helpers.
