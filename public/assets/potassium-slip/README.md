...
Use `runtime-manifest.json` as the Phaser-facing index. It lists texture keys,
spritesheet URLs, frame sizes, frame counts, state names, suggested animations,
and hitbox compatibility notes.

## Enemy Loading

Enemy spritesheets are sized to match the current procedural texture boxes. They
can be loaded under the existing Potassium texture keys before
...
For a minimal drop-in pass, load image replacements for the existing keys:
