...
1. **Phaser world** — a 2D side-scrolling street where the player walks and jumps. World dimensions and physics constants live in [`src/runtime/config.ts`](src/runtime/config.ts); building placements live in [`src/features/overworld/worldLayout.ts`](src/features/overworld/worldLayout.ts) with a compatibility export from [`src/config/worldLayout.ts`](src/config/worldLayout.ts).
