...
- **Side-view player scenes** — new side-view Phaser scenes should use [`src/runtime/player/SideViewPlayerRuntime.ts`](src/runtime/player/SideViewPlayerRuntime.ts) for spawn/resume placement, keyboard/touch input, pause propagation, `PlayerController`, sprite facing/idle animation, appearance sync, and resume capture. Keep scene-specific layout, colliders, prompts, and interactions in the scene or a focused runtime Module.
