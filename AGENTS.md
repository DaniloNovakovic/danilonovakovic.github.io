...
1. **Phaser world** — a 2D side-scrolling street where the player walks and jumps. World dimensions and physics constants live in [`src/game/runtime/config.ts`](src/game/runtime/config.ts); building placements live in [`src/features/overworld/worldLayout.ts`](src/features/overworld/worldLayout.ts).
...
5. **Kernel + scene lifecycle** — [`src/app/modes/interactive/Game.tsx`](src/app/modes/interactive/Game.tsx) boots Phaser, then delegates transitions to [`src/game/core/kernel/GameKernel.ts`](src/game/core/kernel/GameKernel.ts) and [`src/game/core/kernel/SceneManager.ts`](src/game/core/kernel/SceneManager.ts) through [`src/game/infra/phaser/PhaserSceneAdapter.ts`](src/game/infra/phaser/PhaserSceneAdapter.ts). Known Phaser contexts are assembled by [`src/game/contextPlugins/createContextPlugins.ts`](src/game/contextPlugins/createContextPlugins.ts); add new context registration there rather than expanding `Game.tsx`.
6. **Runtime catalogs** — feature-owned catalog entries live in [`src/features/catalog.ts`](src/features/catalog.ts), with compatibility exports in [`src/config/featureRuntimeBindings.ts`](src/config/featureRuntimeBindings.ts) and runtime lookup helpers in [`src/game/runtime/miniGameRegistry.ts`](src/game/runtime/miniGameRegistry.ts). React overlay resolution, parent returns, and Phaser/React kind checks should go through the catalog helpers instead of local maps.
...
- **Kernel event timing** — [`KernelEventBus`](src/game/core/kernel/events.ts) is synchronous and remains the default. [`KernelEventQueue`](src/game/core/kernel/events.ts) is an unwired scaffold for demonstrated time-decoupling needs such as cross-scene side effects, audio one-shots, or analytics; do not use it as a generic global bus.
...
Current ownership: [`src/app`](src/app) holds thin React mode shells, [`src/features`](src/features) holds feature-owned React overlays and presentation modules, [`src/shared`](src/shared) holds shared bridge/UI/hooks, [`src/game/runtime`](src/game/runtime) is the Phaser runtime home, and [`src/game/contextPlugins`](src/game/contextPlugins) holds plugin/context wrappers for kernel orchestration. See [`docs/ARCHITECTURE_RUNTIME.md`](docs/ARCHITECTURE_RUNTIME.md) for the current split.

Use ownership aliases for cross-folder imports: `@app/*`, `@features/*`, `@game/*`, `@shared/*`, and `@config/*`. Keep short local relative imports within a module folder, and import shared UI primitives through `@shared/ui`.
