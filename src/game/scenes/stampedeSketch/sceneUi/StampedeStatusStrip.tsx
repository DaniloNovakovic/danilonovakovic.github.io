...
export function StampedeStatusStrip({ params }: SceneUiSurfaceProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-xl flex-wrap items-center justify-between gap-x-4 gap-y-2 text-[#1a1a1a]">
      <p className="font-mono text-xl font-bold leading-none md:text-2xl">
        {formatTimer(snapshot.timeRemainingSeconds ?? snapshot.timerSeconds ?? 0)}
      </p>
      <div className="min-w-[8rem] flex-1 text-left">
        <p className="font-mono text-xs font-bold uppercase leading-tight tracking-widest text-[#4b4337] md:text-sm">
          {snapshot.phaseLabel ?? 'Kite ideas'}
        </p>
        {feedback && (
          <p className="font-mono text-[10px] font-bold uppercase leading-tight tracking-wider text-[#1a1a1a]/75 md:text-xs">
            {feedback}
          </p>
        )}
      </div>
      <div className="flex min-w-[8rem] items-center gap-2">
        <span className="font-mono text-xs font-bold uppercase tracking-widest text-[#4b4337] md:text-sm">
          Noise
        </span>
        <div
          className="h-3 flex-1 min-w-[4rem] rounded-sm border-2 border-[#1a1a1a] bg-white"
          role="progressbar"
