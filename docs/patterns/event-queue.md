...
Not in use yet. We have two **synchronous** buses:
...
`planned` — specifically as an upgrade path from the synchronous kernel bus when the first real cross-frame/cross-scene case shows up.
