...
- `[KernelEventBus](../../src/game/core/kernel/events.ts)` — typed kernel events (`SceneTransitionRequested`, `OverlayOpened`, `OverlayClosed`, `PauseChanged`), also synchronous.
- `[KernelEventQueue](../../src/game/core/kernel/events.ts)` — a minimal FIFO scaffold covered by tests, not wired into production.
