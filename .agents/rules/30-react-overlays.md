...
1. **No direct Phaser imports in React overlays.** React overlays under `src/features/**` must not import Phaser; the bootstrap component that owns the `Phaser.Game` instance lives under `src/app/modes/interactive`.
...
4. **Use the feature runtime catalog.** React overlay resolution, overlay parent returns, and React/Phaser kind checks should go through [`miniGameRegistry`](../../src/game/runtime/miniGameRegistry.ts), not local maps.
