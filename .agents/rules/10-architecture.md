...
3. **Kernel events are lifecycle notifications, not a generic bus.** Use [`KernelEventBus`](../../src/game/core/kernel/events.ts) for synchronous transition/overlay/pause reactions inside the kernel/runtime seam. Do not use it for durable state, React overlay communication, analytics dumping, or broad app events.
