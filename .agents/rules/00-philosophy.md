...
1. **Use the smallest change inside the existing seam.** Before introducing a new store, bus, global, lifecycle mechanism, or runtime Module, check whether the bridge store, kernel, SceneManager, ECS, or an existing runtime Module already covers the job.
