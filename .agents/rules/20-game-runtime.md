...
13. **Gameplay-critical changes need playability evidence.** If work touches movement constants, blockout topology, anchors, gates, generated colliders, traversal helpers, scene lifecycle, overlay pause, scene returns, shell control mats, mobile input, or mini-game terminal/reward return paths, run or record a scoped playability pass using [`.agents/skills/playability-tester/SKILL.md`](../skills/playability-tester/SKILL.md), or state why the trigger does not apply. Prefer static, deterministic, or targeted harness evidence before Browser/Playwright; browser smoke is for last-mile shell, viewport, and real input integration.
