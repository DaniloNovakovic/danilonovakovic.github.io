...
11. **Scene presentation is shell policy.** Use [`phaserScenePresentation`](../../src/game/runtime/phaserScenePresentation.ts) for `portrait-cover`, `full-board`, and `vertical-board` decisions. Potassium uses `vertical-board`; do not hardcode Potassium-style layout or side-view portrait-cover checks inside unrelated React/Phaser callers.
12. **Do not duplicate runtime policy locally.** Pause/input, resume, scene presentation, side-view camera/player behavior, and interior interactions should use the shared runtime seams above.
